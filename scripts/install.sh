...
install_node_deps() {
    ...
    if [ -f "$INSTALL_DIR/package.json" ]; then
        ...
        # --loglevel=error, not --silent: --silent sets npm's loglevel to
        # "silent", which drops npm's own error text too, defeating the
        # capture below -- a failure would land here with an empty file.
        local npm_log
        npm_log="$(mktemp)"
...
ensure_browser() {
    ...
    # --loglevel=error, not --silent: --silent drops npm's own error text
    # too, so a failure here would be captured to log_file as empty.
    if ! run_with_timeout "$NODE_DEPS_TIMEOUT" "$npm_bin" install -g --prefix "$HERMES_HOME/node" --loglevel=error --ignore-scripts \
